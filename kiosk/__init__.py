...
from services.meals_service import MealDTO
...
@bp.route("/chores/<string:chore_id>/ignore", methods=["POST"], endpoint="ignore_chore")
...
def index():
    ...
    today = date.today()
    chores_today: list[ChoreDTO] = routes.fetch_chores(include_completed=True)
    logger.info("index: loaded %d chores due today", len(chores_today))

    # Fetch meals for the next 7 days
    today = date.today()
    end_date = today + timedelta(days=6)
    meals: list[MealDTO] = routes.fetch_meals(start=today, end=end_date)
...
def view_chores():
    ...
    chores: list[ChoreDTO] = routes.fetch_chores(include_completed=True)
...
@bp.route("/chore/<task_id>/toggle", methods=["POST"])
...
def create_chore():
    ...
    if request.method == "POST":
        ...
        today = date.today()
        tz_name = current_app.config.get("APP_TZ")
...
@bp.post("/chores/<string:chore_id>/complete", endpoint="complete_chore")
...
@bp.post('/admin/chores/<string:task_id>/points', endpoint='admin_set_points')
...
@bp.get('/admin/rewards', endpoint='rewards_page')
def rewards_page():
...
@bp.post('/admin/rewards', endpoint='create_reward')
def create_reward():
...
@bp.post('/admin/redeem', endpoint='redeem_reward')
def redeem_reward():
...
@bp.get('/admin/users', endpoint='admin_users')
def admin_users():
...
@bp.post('/admin/users', endpoint='create_user')
def create_user():
